...
(isLinux ? test.skip : test)(
  "TC-PDF-001-E2E - Complete process should succeed with signature in PDF - ",
  async () => {
    ...
    fs.rmSync(path.join(resultsDir, "signed"), {
      recursive: true,
      force: true,
    });
    ...
    if (diffPixels === 0) {
      ...
      console.log(
        "No visual change detected - moved to 'no-signature' folder.",
      );
