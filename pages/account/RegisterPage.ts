...
export class RegisterPage extends BasePage {
  private readonly goBackToLoginButton = this.page.getByRole('link', { name: 'Back to login' })
  ...
  private readonly emailField = this.page.getByPlaceholder('E-mail address *').first()
  private readonly confirmEmail = this.page.getByPlaceholder('Repeat e-mail address *')
  ...
  private readonly passwordField = this.page.getByPlaceholder('Password *', { exact: true })
  private readonly confirmPassword = this.page.getByPlaceholder('Repeat Password *')
  private readonly policyCheckBox = this.page.locator('#dataProtectionConfirmation');
  ...
  async register(registerData: RegisterData) {
    ...
    await this.birthdayDay.selectOption({ value: `${registerData.birthdayDay}` });
    await this.birthdayMonth.selectOption({ value: `${registerData.birthdayMonth}` });
    await this.birthdayYear.selectOption({ value: `${registerData.birthdayYear}` });
