...
export class BookingPage extends BasePage {
  ...
  async selectFiliale(name: string) {
    ...
    const locationContent = this.page.locator('#step-0 .flex.flex-col.text-myspa-blue').first()
  ...
  async selectPersons(persons: number) {
    ...
    const personSelection = this.page.locator(".text-black.text-md.text-center.font-overpass-light")
    await personSelection.nth(-1 + persons).click()
  }

  async selectDuration(duration: number) {
    this.durationSelector.click()
    const durationSelection = this.page.locator(".cursor-pointer.text-base.text-myspa-blue")
  ...
  async skipInsurance() {
    await expect(this.page.getByText('Continue without insurance')).toBeVisible();
    await this.page.getByRole('button', { name: "continue without insurance" }).click();
  }


  async scrollToCheckout() {
    await this.page.locator('text=Check your information').scrollIntoViewIfNeeded();
  }
  ...
  async confirmBooking() {
    await this.page.getByRole('button', { name: "Pay for your Wellzone now" }).click()
  }
  ...
  async loginToPaypalAndPay() {
    ...
    const confirmBtn = this.page.getByRole('button', { name: 'Weiter zur Überprüfung der Bestellung' });
  ...
  async setCheckinDateFromToday(numberOfDaysFromToday: number) {
    ...
    const checkInDate = this.page.locator('#step-1 .text-myspa-blue.text-md.font-semibold')
  ...
  private async selectDateInTheCalendar(numberOfDaysFromToday: number): Promise<string> {
    ...
    const calendarInput = this.page.locator('.flex.flex-row.self-center').first();
    await calendarInput.scrollIntoViewIfNeeded()

    while (true) {
      const calendarLeft = await this.page.locator('#step-1 .basis-5\\/12.flex.justify-center').first().textContent();
      const calendarRight = await this.page.locator('#step-1 .basis-5\\/12.flex.justify-center').nth(1).textContent();


      if ((calendarLeft?.includes(expectedMonthAndYear)) || (calendarRight?.includes(expectedMonthAndYear))) {
        const matchingDays = this.page.locator('.w-full.flex.flex-row.justify-center.selectable').getByText(`${expectedDate}`, { exact: true });
      ...
      await this.page.locator('.flex.justify-end.cursor-pointer.self-center').first().click();
